...
type Order = {
  ...
  source: 'passenger' | 'waiter'; createdAt: number; promisedAt: number;
};
...
function makeInitialState(): AppState {
  ...
  return {
    ...
    orders: [
      { id: 124, table: 2, seat: 1, source: 'waiter', payment: 'card', comment: 'Морс без льда', items: [{ id: 'borscht', qty: 1 }, { id: 'berry-mors', qty: 1 }], status: 'preparing', kitchenStatus: 'preparing', barStatus: 'ready', createdAt: now - 8 * 60_000, promisedAt: now + 10 * 60_000 },
      { id: 125, table: 1, seat: 3, source: 'passenger', payment: 'cash', comment: '', items: [{ id: 'thyme-tea', qty: 1 }, { id: 'roasted-nuts', qty: 1 }], status: 'pending', kitchenStatus: 'waiting', barStatus: 'waiting', createdAt: now - 2 * 60_000, promisedAt: now + 5 * 60_000 },
    ],
    nextOrderId: 126,
...
export default function Home() {
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    try {
      const saved = localStorage.getItem('rail-dining-demo-v2');
  ...
  useEffect(() => { if (hydrated) localStorage.setItem('rail-dining-demo-v2', JSON.stringify(state)); }, [state, hydrated]);
  ...
  const createOrder = (source: 'passenger' | 'waiter') => {
    ...
    const order: Order = { id, table, seat, items: lines, source, payment: source === 'passenger' ? payment : 'card', comment: source === 'passenger' ? comment.trim() : '', status: isPending ? 'pending' : 'accepted', kitchenStatus: 'waiting', barStatus: 'waiting', createdAt, promisedAt: createdAt + etaFor(lines) * 60_000 };
    setState((current) => ({ ...current, orders: [...current.orders, order], nextOrderId: id + 1, [source === 'passenger' ? 'cart' : 'waiterCart']: {}, events: [addEvent(`Заказ №${id} создан для стола ${table}.`), ...current.events].slice(0, 30) }));
    if (source === 'passenger') { setCheckoutOpen(false); setCartOpen(false); setComment(''); setNotice(isPending ? `Заказ №${id} отправлен официанту` : `Заказ №${id} принят в работу`); }
    else { setNotice(`Заказ №${id} создан для стола ${table}`); setWaiterTab('active'); }
  };
  ...
  const callService = (type: ServiceType) => {
    ...
    setState((current) => ({ ...current, calls: [...current.calls, { id, table: 4, type, status: 'new', createdAt: Date.now() }], nextCallId: id + 1, events: [addEvent(`Стол 4: ${type === 'waiter' ? 'вызов официанта' : 'запрос счёта'}.`), ...current.events].slice(0, 30) }));
  ...
  const renderCart = ({ waiter = false }: { waiter?: boolean }) => {
    ...
    return <aside className={`cart-panel ${waiter ? 'waiter-cart' : ''}`}><div className="panel-heading"><div><span className="eyebrow">{waiter ? `Стол ${selectedTable} · место ${selectedSeat}` : 'Стол 4'}</span><h2>Ваш заказ</h2></div>{!waiter && <button className="icon-button close-mobile" onClick={() => setCartOpen(false)}>×</button>}</div>{!lines.length ? <EmptyState icon="＋" title="Корзина пуста" text="Добавьте блюда из меню — они появятся здесь." /> : <><div className="cart-lines">{lines.map((line) => { const item = itemFor(line.id)!; return <div className="cart-line" key={line.id}><div><strong>{item.name}</strong><small>{money(item.price)} · {item.minutes} мин</small></div><Stepper value={line.qty} compact onChange={(value) => changeCart(line.id, value, waiter)} /></div>; })}</div><div className="cart-summary"><div><span>Позиций</span><strong>{lines.reduce((sum, line) => sum + line.qty, 0)}</strong></div><div><span>Ожидаемое время</span><strong>до {etaFor(lines)} мин</strong></div><div className="cart-total"><span>Итого</span><strong>{money(totalFor(lines))}</strong></div></div><button className="primary-button full" onClick={() => waiter ? createOrder('waiter') : setCheckoutOpen(true)}>{waiter ? 'Принять заказ' : 'Оформить заказ'}</button></>}</aside>;
  };
  ...
  const renderOrderCard = ({ order, station }: { order: Order; station?: Station }) => {
    ...
    return <article className={`order-card ${countdown.late ? 'order-late' : ''}`}><div className="order-card-head"><div><span className="order-number">№ {order.id}</span><h3>Стол {order.table} · место {order.seat}</h3></div>{station ? <span className={`status station-${stationFor(order, station)}`}>{stationLabels[stationFor(order, station)]}</span> : <MiniStatus status={order.status} />}</div><div className="ticket-time"><span>{stamp(new Date(order.createdAt))}</span><strong className={countdown.late ? 'late' : ''}>{countdown.text}</strong></div><div className="ticket-lines">{visibleLines.map((line) => <div key={line.id}><strong>{line.qty}×</strong><span>{itemFor(line.id)?.name}</span></div>)}</div>{order.comment && <div className="order-comment">“{order.comment}”</div>}{!station && <div className="split-progress">{hasStation(order.items, 'kitchen') && <span><i className={`dot ${order.kitchenStatus}`} /> Кухня: {stationLabels[order.kitchenStatus]}</span>}{hasStation(order.items, 'bar') && <span><i className={`dot ${order.barStatus}`} /> Бар: {stationLabels[order.barStatus]}</span>}</div>}{station && stationFor(order, station) === 'waiting' && <button className="secondary-button full" onClick={() => setStationStatus(order.id, station, 'preparing')}>Начать готовить</button>}{station && stationFor(order, station) === 'preparing' && <button className="primary-button full" onClick={() => setStationStatus(order.id, station, 'ready')}>Готово к выдаче</button>}{!station && order.status === 'ready' && <button className="primary-button full" onClick={() => markServed(order.id)}>Отметить «Подан»</button>}</article>;
  };

  const renderPassengerView = () => <div className="passenger-view">
    <section className="passenger-hero"><div><span className="eyebrow">Вагон-ресторан · стол 4</span><h1>Добро пожаловать</h1><p>Выберите блюда — мы приготовим их, пока за окном меняются города.</p></div><div className="service-actions"><button onClick={() => callService('waiter')}>◇ Позвать официанта</button><button onClick={() => callService('bill')}>▤ Попросить счёт</button></div></section>
    {passengerOrders.length > 0 && <section className="passenger-orders">{passengerOrders.slice(0, 2).map((order) => { const countdown = remaining(order.promisedAt, clock); return <div className="passenger-order" key={order.id}><div><span>Заказ №{order.id}</span><strong>{orderLabels[order.status]}</strong><small>{order.status === 'pending' ? 'Официант проверит состав перед отправкой' : order.status === 'ready' ? 'Официант скоро принесёт заказ' : order.status === 'served' ? 'Приятного аппетита!' : `Ожидаемое время: ${countdown.text}`}</small></div><div className={`progress-ring status-${order.status}`}><b>{order.status === 'ready' ? '✓' : order.status === 'pending' ? '…' : order.status === 'served' ? '✓' : Math.max(0, Math.ceil((order.promisedAt - clock) / 60_000))}</b><small>{['ready', 'served'].includes(order.status) ? '' : 'мин'}</small></div></div>; })}</section>}
    <section className="menu-toolbar"><div className="search-box"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти блюдо или напиток" /></div><div className="category-scroll">{categories.map((name) => <button key={name} className={category === name ? 'active' : ''} onClick={() => setCategory(name)}>{name}</button>)}</div></section>
    ...
    {cartCount > 0 && <button className="mobile-cart-button" onClick={() => setCartOpen(true)}><span>Корзина · {cartCount}</span><strong>{money(totalFor(linesFrom(state.cart)))}</strong></button>}
