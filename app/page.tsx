...
export default function Home() {
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    try {
      const saved = localStorage.getItem('rail-dining-demo-v4');
  ...
  useEffect(() => { if (hydrated) localStorage.setItem('rail-dining-demo-v4', JSON.stringify(state)); }, [state, hydrated]);
  ...
  const adjustKitchenTime = (orderId: number, delta: number) => setState((current) => ({ ...current,
    orders: current.orders.map((order) => {
      if (order.id !== orderId) return order;
      const base = order.kitchenEtaMinutes ?? etaFor(order.items.filter((line) => itemFor(line.id)?.station === 'kitchen'));
      const kitchenEtaMinutes = Math.min(60, Math.max(5, base + delta));
      return { ...order, kitchenEtaMinutes, promisedAt: clock + kitchenEtaMinutes * 60_000 };
    }),
    events: [addEvent(`Кухня скорректировала время заказа №${orderId} на ${delta > 0 ? '+' : ''}${delta} мин.`), ...current.events].slice(0, 30),
  }));
  ...
  const startNewWaiterOrder = () => {
    ...
    setState((current) => ({ ...current, waiterCart: {} }));
  ...
  const renderDishCard = ({ item, waiter = false }: { item: MenuItem; waiter?: boolean }) => {
    ...
    return <article className={`dish-card ${stopped ? 'dish-stopped' : ''} ${waiter ? 'dish-card-compact' : ''}`}><div className="dish-image-wrap"><img src={`${basePath}/${item.image}`} alt={item.name} className="dish-image" /><button className="dish-info-button" onClick={() => setInfoItemId(item.id)} aria-label={`Информация о блюде ${item.name}`}>i</button>{item.popular && <span className="dish-badge">Выбор гостей</span>}{stopped && <span className="stop-badge">Скоро появится</span>}</div><div className="dish-body"><div className="dish-meta"><span>{item.category}</span><span>≈ {item.minutes} мин</span></div><h3>{item.name}</h3>{!waiter && <p>{item.description}</p>}<div className="dish-footer"><div><strong>{money(item.price)}</strong><small>{item.weight}</small></div>{qty > 0 ? <Stepper value={qty} compact onChange={(value) => changeCart(item.id, value, waiter)} /> : <button className="add-button" disabled={stopped} onClick={() => changeCart(item.id, 1, waiter)} aria-label={`Добавить ${item.name}`}>+</button>}</div></div></article>;
  };
  ...
  const renderWaiterMenuItem = (item: MenuItem) => {
    ...
    return <article className={`waiter-menu-item ${stopped ? 'dish-stopped' : ''}`} key={item.id}>
      ...
      <div className="waiter-item-actions"><button className="waiter-info" onClick={() => setInfoItemId(item.id)} aria-label={`Информация о блюде ${item.name}`}>i</button>{qty > 0 ? <Stepper value={qty} compact onChange={(value) => changeCart(item.id, value, true)} /> : <button className="waiter-add" disabled={stopped} onClick={() => changeCart(item.id, 1, true)} aria-label={`Добавить ${item.name}`}>+</button>}</div>
    </article>;
  };
  ...
  const renderCart = ({ waiter = false }: { waiter?: boolean }) => {
    ...
    return <aside className={`cart-panel ${waiter ? 'waiter-cart' : ''}`}>
      ...
      {!lines.length ? <EmptyState icon="＋" title="Корзина пуста" text="Добавьте блюда из меню — они появятся здесь." /> : <>
        ...
        {!waiter && recommendations.length > 0 && <section className="cart-recommendations"><div><span>Возможно, стоит присмотреться</span><small>Часто добавляют к заказу</small></div>{recommendations.map((item) => <button key={item.id} onClick={() => changeCart(item.id, 1)}><span><strong>{item.name}</strong><small>{money(item.price)}</small></span><b>＋</b></button>)}</section>}
  ...
  const renderOrderCard = ({ order, station }: { order: Order; station?: Station }) => {
    ...
    const kitchenEta = order.kitchenEtaMinutes ?? etaFor(order.items.filter((line) => itemFor(line.id)?.station === 'kitchen'));
    return <article className={`order-card ${countdown.late ? 'order-late' : ''}`}>
      ...
      {station === 'kitchen' && stationFor(order, station) !== 'ready' && <div className="kitchen-time-control"><span>Время кухни</span><div><button onClick={() => adjustKitchenTime(order.id, -5)} aria-label="Уменьшить время приготовления">−</button><strong>{kitchenEta} мин</strong><button onClick={() => adjustKitchenTime(order.id, 5)} aria-label="Увеличить время приготовления">+</button></div></div>}
