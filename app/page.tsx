...
type MenuItem = {
  ...
  ingredients: string; calories: number; protein: number; fat: number; carbs: number;
};
type OrderLine = { id: string; qty: number; comboSelections?: ComboSelections };
type Order = {
  ...
  rejectionReason?: string; kitchenReadyAt?: number; barReadyAt?: number; kitchenEtaMinutes?: number;
};
type Visit = {
...
type AppState = {
  ...
  waiterCart: Record<string, number>; cartComboSelections: Record<string, ComboSelections>;
  waiterComboSelections: Record<string, ComboSelections>; prepTimes: Record<string, number>;
...
const categories = ['Популярное', 'Комбо', 'Супы', 'Горячее', 'Закуски', 'Десерты', 'Напитки', 'Снеки'];
const configurableComboIds = new Set(['aurora-lunch-combo']);
const comboSections = [
  { id: 'first', title: 'Первое блюдо', hint: 'Выберите один суп', options: ['borscht', 'chicken-noodle-soup', 'mushroom-cream-soup', 'solyanka', 'ukha'] },
  { id: 'second', title: 'Второе блюдо', hint: 'Выберите одно горячее', options: ['pozharsky-cutlet', 'beef-stroganoff', 'salmon', 'pot-roast', 'pelmeni'] },
  { id: 'salad', title: 'Салат или закуска', hint: 'Выберите одну позицию', options: ['olivier', 'beet-salad', 'herring-potatoes', 'salmon-bruschetta', 'cheese-board'] },
  { id: 'drink', title: 'Напиток', hint: 'Выберите один напиток', options: ['berry-mors', 'thyme-tea', 'cappuccino', 'sea-buckthorn-lemonade', 'mineral-water'] },
  { id: 'bread', title: 'Хлебобулочная продукция', hint: 'Выберите хлеб или выпечку', options: ['rye-crackers', 'mini-pretzels', 'salmon-bruschetta', 'apple-strudel', 'medovik'] },
] as const;
...
const linesFrom = (cart: Record<string, number>, combos: Record<string, ComboSelections> = {}) => Object.entries(cart).filter(([, qty]) => qty > 0).map(([id, qty]) => ({ id, qty, ...(combos[id] ? { comboSelections: combos[id] } : {}) }));
const totalFor = (lines: OrderLine[]) => lines.reduce((sum, line) => sum + (itemFor(line.id)?.price ?? 0) * line.qty, 0);
const idsForLine = (line: OrderLine) => [line.id, ...Object.values(line.comboSelections ?? {})];
const hasStation = (lines: OrderLine[], station: Station) => lines.some((line) => idsForLine(line).some((id) => itemFor(id)?.station === station));
const stationTotalFor = (orders: Order[], station: Station) => orders.reduce((sum, order) => sum + order.items.reduce((lineSum, line) => {
  const stations = (['kitchen', 'bar'] as Station[]).filter((target) => hasStation([line], target));
  return stations.includes(station) ? lineSum + ((itemFor(line.id)?.price ?? 0) * line.qty) / stations.length : lineSum;
}, 0), 0);
const stationFor = (order: Order, station: Station) => station === 'kitchen' ? order.kitchenStatus : order.barStatus;
const readyAtFor = (order: Order, station: Station) => station === 'kitchen' ? order.kitchenReadyAt : order.barReadyAt;
const callWaitLevel = (createdAt: number, current: number) => {
  const minutes = Math.floor((current - createdAt) / 60_000);
  return minutes < 4 ? 'green' : minutes < 7 ? 'yellow' : 'red';
};
const callWaitText = (createdAt: number, current: number) => {
  const minutes = Math.max(0, Math.floor((current - createdAt) / 60_000));
...
function makeInitialState(): AppState {
  ...
  return {
    ...
    cartComboSelections: {}, waiterComboSelections: {}, prepTimes: {},
    visits: [
    ...
    orders: [
      { id: 124, table: 2, seat: 1, visitId: 1, source: 'waiter', payment: 'card', comment: 'Морс без льда', items: [{ id: 'borscht', qty: 1 }, { id: 'berry-mors', qty: 1 }], status: 'preparing', kitchenStatus: 'preparing', barStatus: 'ready', barReadyAt: now - 60_000, createdAt: now - 8 * 60_000, promisedAt: now + 10 * 60_000 },
...
export default function Home() {
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    try {
      const saved = localStorage.getItem('rail-dining-demo-v5');
  ...
  const [waiterCheckoutOpen, setWaiterCheckoutOpen] = useState(false);
  ...
  const [clearTableId, setClearTableId] = useState<number | null>(null);
  ...
  const [comboModal, setComboModal] = useState<{ id: string; waiter: boolean } | null>(null);
  const [comboDraft, setComboDraft] = useState<ComboSelections>({});

  useEffect(() => {
    const ready = window.setTimeout(() => setHydrated(true), 0);
    return () => window.clearTimeout(ready);
  }, []);
  useEffect(() => { if (hydrated) localStorage.setItem('rail-dining-demo-v5', JSON.stringify(state)); }, [state, hydrated]);
  ...
  const effectiveMinutes = (id: string) => state.prepTimes?.[id] ?? itemFor(id)?.minutes ?? 0;
  const etaForLines = (lines: OrderLine[]) => Math.max(1, ...lines.flatMap((line) => idsForLine(line).map(effectiveMinutes)));
  const lineUnavailable = (line: OrderLine) => idsForLine(line).some((id) => state.stopList.includes(id));
  const openCombo = (id: string, waiter = false) => {
    const current = waiter ? state.waiterComboSelections?.[id] : state.cartComboSelections?.[id];
    setComboDraft(current ? { ...current } : {}); setComboModal({ id, waiter });
  };
  const saveCombo = () => {
    if (!comboModal || !comboSections.every((section) => comboDraft[section.id])) return;
    const cartKey = comboModal.waiter ? 'waiterCart' : 'cart';
    const comboKey = comboModal.waiter ? 'waiterComboSelections' : 'cartComboSelections';
    setState((current) => ({ ...current,
      [cartKey]: { ...current[cartKey], [comboModal.id]: Math.max(1, current[cartKey][comboModal.id] ?? 0) },
      [comboKey]: { ...(current[comboKey] ?? {}), [comboModal.id]: { ...comboDraft } },
    }));
    setComboModal(null); setComboDraft({}); setNotice('Комбо настроено и добавлено в заказ');
  };
  const adjustPrepTime = (id: string, delta: number) => setState((current) => {
    const base = current.prepTimes?.[id] ?? itemFor(id)?.minutes ?? 5;
    const next = Math.min(60, Math.max(1, base + delta));
    return { ...current, prepTimes: { ...(current.prepTimes ?? {}), [id]: next }, events: [addEvent(`Кухня изменила время «${itemFor(id)?.name}» на ${next} мин.`), ...current.events].slice(0, 30) };
  });

  const createOrder = (source: 'passenger' | 'waiter') => {
    const cart = source === 'passenger' ? state.cart : state.waiterCart;
    const combos = source === 'passenger' ? state.cartComboSelections : state.waiterComboSelections;
    const lines = linesFrom(cart, combos ?? {}).filter((line) => !lineUnavailable(line));
    if (!lines.length) return;
    const table = source === 'passenger' ? 4 : selectedTable;
    const seat = source === 'passenger' ? passengerSeat : selectedSeat;
    const id = state.nextOrderId;
    const openVisitCandidate = state.visits.find((visit) => visit.table === table && visit.status !== 'closed');
    const openVisit = openVisitCandidate && state.orders.some((order) => order.visitId === openVisitCandidate.id && order.status !== 'rejected') ? openVisitCandidate : undefined;
    const visitId = openVisit?.id ?? state.nextVisitId;
    const rootOrder = openVisit ? state.orders.filter((order) => order.visitId === openVisit.id && order.status !== 'rejected').sort((a, b) => a.createdAt - b.createdAt)[0] : undefined;
    ...
    const order: Order = { id, table, seat, visitId, parentId: rootOrder?.id, items: lines, source, payment: orderPayment, comment: source === 'passenger' ? comment.trim() : waiterComment.trim(), status: isPending ? 'pending' : 'accepted', kitchenStatus: 'waiting', barStatus: 'waiting', createdAt, promisedAt: createdAt + etaForLines(lines) * 60_000 };
    ...
    setState((current) => ({ ...current, visits: newVisit ? [...current.visits.map((visit) => visit.table === table && visit.status !== 'closed' ? { ...visit, status: 'closed' as VisitStatus, closedAt: createdAt } : visit), newVisit] : current.visits.map((visit) => visit.id === visitId && visit.status === 'bill' ? { ...visit, status: 'open' } : visit), nextVisitId: newVisit ? visitId + 1 : current.nextVisitId, orders: [...current.orders, order], nextOrderId: id + 1, [source === 'passenger' ? 'cart' : 'waiterCart']: {}, [source === 'passenger' ? 'cartComboSelections' : 'waiterComboSelections']: {}, events: [addEvent(`${rootOrder ? 'Дозаказ' : 'Заказ'} №${id} создан для стола ${table}.`), ...current.events].slice(0, 30) }));
    ...
    else { setWaiterComment(''); setWaiterCartOpen(false); setWaiterCheckoutOpen(false); setNotice(rootOrder ? `Дозаказ №${id} создан для стола ${table}` : `Заказ №${id} создан для стола ${table}`); setWaiterTab('active'); }
  };

  const openReview = (order: Order) => { setReviewId(order.id); setReviewDraft(Object.fromEntries(order.items.map((line) => [line.id, line.qty]))); setRejectReason(''); };
  const approveReview = () => {
    const original = state.orders.find((order) => order.id === reviewId);
    const lines = linesFrom(reviewDraft).map((line) => ({ ...line, ...(original?.items.find((item) => item.id === line.id)?.comboSelections ? { comboSelections: original.items.find((item) => item.id === line.id)?.comboSelections } : {}) }));
    if (!reviewId || !lines.length || lines.some(lineUnavailable)) return;
    const approvedAt = Date.now();
    setState((current) => ({ ...current, orders: current.orders.map((order) => order.id === reviewId ? { ...order, items: lines, status: 'accepted', kitchenStatus: 'waiting', barStatus: 'waiting', promisedAt: approvedAt + etaForLines(lines) * 60_000 } : order), events: [addEvent(`Официант подтвердил заказ №${reviewId}.`), ...current.events].slice(0, 30) }));
    setReviewId(null); setReviewDraft({}); setNotice(`Заказ №${reviewId} подтверждён`);
  };
  const rejectReview = () => {
    if (!reviewId || !rejectReason.trim()) { setNotice('Укажите причину отклонения заказа'); return; }
    setState((current) => {
      const target = current.orders.find((order) => order.id === reviewId);
      const hasAnotherActiveOrder = target ? current.orders.some((order) => order.visitId === target.visitId && order.id !== reviewId && order.status !== 'rejected') : false;
      return { ...current,
        orders: current.orders.map((order) => order.id === reviewId ? { ...order, status: 'rejected', rejectionReason: rejectReason.trim() } : order),
        visits: target && !hasAnotherActiveOrder ? current.visits.map((visit) => visit.id === target.visitId ? { ...visit, status: 'closed', closedAt: clock } : visit) : current.visits,
        events: [addEvent(`Заказ №${reviewId} отменён: ${rejectReason.trim()}.`), ...current.events].slice(0, 30),
      };
    });
    setReviewId(null); setReviewDraft({}); setRejectReason(''); setNotice(`Заказ №${reviewId} отменён`);
  };
  const setStationStatus = (orderId: number, station: Station, value: StationStatus) => setState((current) => ({ ...current, orders: current.orders.map((order) => { if (order.id !== orderId) return order; const statusKey = station === 'kitchen' ? 'kitchenStatus' : 'barStatus'; const readyKey = station === 'kitchen' ? 'kitchenReadyAt' : 'barReadyAt'; const changed = { ...order, [statusKey]: value, [readyKey]: value === 'ready' ? clock : undefined } as Order; return { ...changed, status: resolvedStatus(changed) }; }), events: [addEvent(`${station === 'kitchen' ? 'Кухня' : 'Бар'}: заказ №${orderId} — ${stationLabels[value].toLocaleLowerCase('ru-RU')}.`), ...current.events].slice(0, 30) }));
  const returnToStation = (orderId: number, station: Station) => setStationStatus(orderId, station, 'preparing');
  ...
  const startNewWaiterOrder = () => {
    const freeTable = [1, 2, 3, 4].find((table) => !state.visits.some((visit) => visit.table === table && visit.status !== 'closed')) ?? selectedTable;
    setSelectedTable(freeTable); setSelectedSeat(1); setWaiterTab('create'); setWaiterSearch(''); setWaiterCategory('Популярное'); setWaiterComment(''); setWaiterCartOpen(false);
    setState((current) => ({ ...current, waiterCart: {}, waiterComboSelections: {} }));
    window.setTimeout(() => document.getElementById('waiter-order-editor')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
  };
  const clearTableHistory = () => {
    if (!clearTableId) return;
    if (clearTablePassword !== '7420') { setNotice('Неверный пароль'); return; }
    setState((current) => {
      const visitIds = current.visits.filter((visit) => visit.table === clearTableId && visit.status === 'closed').map((visit) => visit.id);
      return { ...current,
        visits: current.visits.filter((visit) => !visitIds.includes(visit.id)),
        orders: current.orders.filter((order) => !visitIds.includes(order.visitId)),
        calls: current.calls.filter((call) => !(call.table === clearTableId && call.status === 'accepted')),
        events: [addEvent(`Официант очистил историю стола ${clearTableId}.`), ...current.events].slice(0, 30),
      };
    });
    setClearTableId(null); setClearTablePassword(''); setNotice(`История стола ${clearTableId} очищена`);
  };
  const reviewOrder = reviewId ? state.orders.find((order) => order.id === reviewId) : null;
  const infoItem = infoItemId ? itemFor(infoItemId) : null;
  const comboItem = comboModal ? itemFor(comboModal.id) : null;
  const passengerCartLines = linesFrom(state.cart, state.cartComboSelections ?? {});
  const waiterCheckoutLines = linesFrom(state.waiterCart, state.waiterComboSelections ?? {});
  const reviewLines = reviewOrder ? linesFrom(reviewDraft).map((line) => ({ ...line, ...(reviewOrder.items.find((item) => item.id === line.id)?.comboSelections ? { comboSelections: reviewOrder.items.find((item) => item.id === line.id)?.comboSelections } : {}) })) : [];
  ...
  const renderDishCard = ({ item, waiter = false }: { item: MenuItem; waiter?: boolean }) => {
    ...
    const configurable = configurableComboIds.has(item.id);
    return <article className={`dish-card ${stopped ? 'dish-stopped' : ''} ${waiter ? 'dish-card-compact' : ''}`}><div className="dish-image-wrap"><img src={`${basePath}/${item.image}`} alt={item.name} className="dish-image" /><button className="dish-info-button" onClick={() => setInfoItemId(item.id)} aria-label={`Информация о блюде ${item.name}`}>i</button>{item.popular && <span className="dish-badge">Выбор гостей</span>}{stopped && <span className="stop-badge">Скоро появится</span>}</div><div className="dish-body"><div className="dish-meta"><span>{item.category}</span><span>≈ {effectiveMinutes(item.id)} мин</span></div><h3>{item.name}</h3>{!waiter && <p>{item.description}</p>}<div className="dish-footer"><div><strong>{money(item.price)}</strong><small>{qty > 0 && configurable ? `В заказе: ${qty}` : item.weight}</small></div>{configurable ? <button className="configure-button" disabled={stopped} onClick={() => openCombo(item.id, waiter)}>{qty > 0 ? 'Изменить' : 'Настроить'}</button> : qty > 0 ? <Stepper value={qty} compact onChange={(value) => changeCart(item.id, value, waiter)} /> : <button className="add-button" disabled={stopped} onClick={() => changeCart(item.id, 1, waiter)} aria-label={`Добавить ${item.name}`}>+</button>}</div></div></article>;
  };
  ...
  const renderWaiterMenuItem = (item: MenuItem) => {
    ...
    return <article className={`waiter-menu-item ${stopped ? 'dish-stopped' : ''}`} key={item.id}>
      <div className="waiter-menu-thumb"><img src={`${basePath}/${item.image}`} alt="" />{stopped && <span>Скоро появится</span>}</div>
      <div className="waiter-menu-copy"><span>{item.category} · {effectiveMinutes(item.id)} мин</span><strong>{item.name}</strong><small>{item.weight} · {money(item.price)}</small></div>
      <div className="waiter-item-actions"><button className="waiter-info" onClick={() => setInfoItemId(item.id)} aria-label={`Информация о блюде ${item.name}`}>i</button>{configurable ? <button className="waiter-configure" disabled={stopped} onClick={() => openCombo(item.id, true)}>{qty > 0 ? 'Изм.' : 'Настр.'}</button> : qty > 0 ? <Stepper value={qty} compact onChange={(value) => changeCart(item.id, value, true)} /> : <button className="waiter-add" disabled={stopped} onClick={() => changeCart(item.id, 1, true)} aria-label={`Добавить ${item.name}`}>+</button>}</div>
    </article>;
  };

  const renderCart = ({ waiter = false }: { waiter?: boolean }) => {
    const cart = waiter ? state.waiterCart : state.cart;
    const combos = waiter ? state.waiterComboSelections : state.cartComboSelections;
    const lines = linesFrom(cart, combos ?? {});
    const table = waiter ? selectedTable : 4;
    const openVisit = state.visits.find((visit) => visit.table === table && visit.status !== 'closed');
    const isAddon = Boolean(openVisit && state.orders.some((order) => order.visitId === openVisit.id && order.status !== 'rejected'));
    const cartComment = waiter ? waiterComment : comment;
    const recommendations = ['thyme-tea', 'rye-crackers', 'mini-pretzels', 'medovik'].map(itemFor).filter((item): item is MenuItem => Boolean(item && !cart[item.id] && !state.stopList.includes(item.id))).slice(0, 3);
    return <aside className={`cart-panel ${waiter ? 'waiter-cart' : ''}`}>
      <div className="panel-heading"><div><span className="eyebrow">{waiter ? `Стол ${selectedTable} · место ${selectedSeat}` : 'Стол 4'}</span><h2>{waiter ? (isAddon ? 'Дозаказ' : 'Новый заказ') : (isAddon ? 'Дозаказ' : 'Ваш заказ')}</h2></div>{!waiter && <button className="icon-button close-mobile" onClick={() => setCartOpen(false)}>×</button>}</div>
      {!lines.length ? <EmptyState icon="＋" title="Корзина пуста" text="Добавьте блюда из меню — они появятся здесь." /> : <>
        <div className="cart-lines">{lines.map((line) => { const item = itemFor(line.id)!; const comboNames = Object.values(line.comboSelections ?? {}).map((id) => itemFor(id)?.name).filter(Boolean); return <div className="cart-line" key={line.id}><button className="cart-line-image" onClick={() => setInfoItemId(item.id)} aria-label={`Открыть информацию о ${item.name}`}><img src={`${basePath}/${item.image}`} alt="" /></button><button className="cart-line-copy" onClick={() => setInfoItemId(item.id)}><strong>{item.name}</strong><small>{money(item.price)} · {effectiveMinutes(item.id)} мин</small>{comboNames.length > 0 && <em>{comboNames.join(' · ')}</em>}</button>{configurableComboIds.has(line.id) && <button className="cart-combo-edit" onClick={() => openCombo(line.id, waiter)}>Настроить</button>}<Stepper value={line.qty} compact onChange={(value) => changeCart(line.id, value, waiter)} /></div>; })}</div>
        ...
        <div className="cart-summary"><div><span>Позиций</span><strong>{lines.reduce((sum, line) => sum + line.qty, 0)}</strong></div><div><span>Ожидаемое время</span><strong>до {etaForLines(lines)} мин</strong></div><div className="cart-total"><span>Итого</span><strong>{money(totalFor(lines))}</strong></div></div>
        {!waiter && recommendations.length > 0 && <section className="cart-recommendations"><div><span>Возможно, стоит присмотреться</span><small>Листайте подборку — карточка открывает состав</small></div><div className="recommendations-track">{recommendations.map((item) => <article key={item.id}><button className="recommendation-info" onClick={() => setInfoItemId(item.id)}><img src={`${basePath}/${item.image}`} alt="" /><span><strong>{item.name}</strong><small>{money(item.price)}</small></span></button><button className="recommendation-add" onClick={() => changeCart(item.id, 1)} aria-label={`Добавить ${item.name}`}>＋</button></article>)}</div></section>}
        {lines.some(lineUnavailable) && <div className="warning-note cart-warning">Одна из выбранных позиций временно недоступна. Измените состав заказа.</div>}
        <button className="primary-button full" disabled={lines.some(lineUnavailable)} onClick={() => { if (waiter) { setWaiterCartOpen(false); setWaiterCheckoutOpen(true); } else setCheckoutOpen(true); }}>{waiter ? 'Проверить новый заказ' : (isAddon ? 'Оформить дозаказ' : 'Оформить заказ')}</button>
      </>}
    </aside>;
  };

  const renderOrderCard = ({ order, station }: { order: Order; station?: Station }) => {
    const visibleLines = station ? order.items.filter((line) => idsForLine(line).some((id) => itemFor(id)?.station === station)) : order.items;
    const countdown = remaining(order.promisedAt, clock);
    const visit = state.visits.find((item) => item.id === order.visitId);
    const readyAt = station ? readyAtFor(order, station) : undefined;
    const canReturn = Boolean(station && stationFor(order, station) === 'ready' && readyAt && clock - readyAt <= 2 * 60_000);
    return <article className={`order-card ${countdown.late ? 'order-late' : ''}`}>
      ...
      <div className="ticket-time"><span>{stamp(new Date(order.createdAt))}</span><strong className={!['served', 'rejected'].includes(order.status) && countdown.late ? 'late' : ''}>{order.status === 'served' ? 'подан' : order.status === 'rejected' ? 'отменён' : countdown.text}</strong></div>
      <div className="ticket-lines">{visibleLines.map((line) => { const details = Object.values(line.comboSelections ?? {}).filter((id) => !station || itemFor(id)?.station === station).map((id) => itemFor(id)?.name).filter(Boolean); return <div key={line.id}><strong>{line.qty}×</strong><span>{itemFor(line.id)?.name}</span>{details.length > 0 && <small>{details.join(' · ')}</small>}</div>; })}</div>
      {order.comment && <div className="order-comment">Комментарий: “{order.comment}”</div>}
      {order.status === 'rejected' && <div className="rejection-note">Причина: {order.rejectionReason || 'заказ отклонён официантом'}</div>}
      ...
      {station && canReturn && <button className="return-station-button full" onClick={() => returnToStation(order.id, station)}>{station === 'kitchen' ? '↶ Вернуть на кухню' : '↶ Вернуть в бар'}<small>доступно 2 минуты после «Готово»</small></button>}
  ...
  const renderPassengerView = () => {
    ...
    return <div className="passenger-view">
      <section className="passenger-hero"><div><span className="eyebrow">Вагон-ресторан · стол 4</span><h1>Добро пожаловать</h1><p><span>{trainRoute}</span><br />Выберите блюда — мы приготовим их, пока за окном меняются города.</p></div><div className="service-actions"><button onClick={() => callService('waiter')}><span aria-hidden="true">✋</span> Позвать официанта</button><button onClick={() => callService('bill')}><span aria-hidden="true">💳</span> Попросить счёт</button></div></section>
      ...
      {passengerOrders.length > 0 && <section className="passenger-orders">{passengerOrders.slice(0, 3).map((order) => { const countdown = remaining(order.promisedAt, clock); const rejected = order.status === 'rejected'; const statusText = rejected ? `Причина: ${order.rejectionReason || 'заказ отклонён официантом'}` : order.status === 'pending' ? 'Официант проверит состав перед отправкой' : order.status === 'ready' ? 'Официант скоро принесёт заказ' : order.status === 'served' ? 'Приятного аппетита!' : `Ожидаемое время: ${countdown.text}`; return <div className="passenger-order" key={order.id}><div><span>{order.parentId ? `Дозаказ №${order.id} к №${order.parentId}` : `Заказ №${order.id}`}</span><strong>{orderLabels[order.status]}</strong><small>{statusText}</small></div><div className={`progress-ring status-${order.status}`}><b>{rejected ? '×' : order.status === 'ready' ? '✓' : order.status === 'pending' ? '…' : order.status === 'served' ? '✓' : Math.max(0, Math.ceil((order.promisedAt - clock) / 60_000))}</b><small>{rejected || ['ready', 'served'].includes(order.status) ? '' : 'мин'}</small></div></div>; })}</section>}
      ...
      {cartCount > 0 && <button className="mobile-cart-button" onClick={() => setCartOpen(true)}><span>{passengerVisit ? 'Дозаказ' : 'Корзина'} · {cartCount}</span><strong>{money(totalFor(linesFrom(state.cart, state.cartComboSelections ?? {})))}</strong></button>}
  ...
  const renderWaiterView = () => {
    ...
    const selectedClosedHistory = !selectedVisit && state.visits.some((visit) => visit.table === selectedTable && visit.status === 'closed');
    ...
    return <div className="staff-view waiter-view"><section className="staff-hero"><div><span className="eyebrow">Смена · зал вагона-ресторана</span><h1>Официант</h1><p>{pendingCount} на подтверждение · {readyCount} готовы · {openCalls.length} запросов гостей</p></div><div className="hero-clock"><strong>{stamp(new Date(clock))}</strong><span>автономный режим</span></div></section>
      {openCalls.length > 0 && <div className="call-strip">{[...openCalls].sort((a, b) => a.createdAt - b.createdAt).map((call) => <div key={call.id} className={`call-age-${callWaitLevel(call.createdAt, clock)}`}><span>{call.type === 'bill' ? '💳' : '✋'}</span><strong>Стол {call.table}: {call.type === 'bill' ? 'подать счёт' : 'подойти к столу'}<small>ожидают {callWaitText(call.createdAt, clock)}</small></strong><button onClick={() => acceptCall(call.id)}>Принять</button></div>)}</div>}
      <section className="table-map">
        ...
        <div className="tables-row">{[1, 2, 3, 4].map((table) => { const visit = state.visits.find((item) => item.table === table && item.status !== 'closed'); const visitOrders = visit ? state.orders.filter((order) => order.visitId === visit.id && order.status !== 'rejected') : []; const calls = openCalls.filter((call) => call.table === table).sort((a, b) => a.createdAt - b.createdAt); const oldestCall = calls[0]; const needsClosing = visitOrders.length > 0 && visitOrders.every((order) => order.status === 'served'); const attention = visitOrders.some((order) => ['pending', 'ready'].includes(order.status)) || calls.length > 0 || needsClosing; const count = visitOrders.length + calls.length; const callClass = oldestCall ? `call-age-${callWaitLevel(oldestCall.createdAt, clock)}` : ''; return <button key={table} className={`${selectedTable === table ? 'selected' : ''} ${attention ? 'needs-attention' : ''} ${callClass}`} onClick={() => setSelectedTable(table)} aria-label={`Стол ${table}: ${count ? `${count} событий` : 'свободен'}`}><span>Стол</span><strong>{table}</strong><small>{oldestCall ? oldestCall.type === 'bill' ? 'подать счёт' : 'подойти к столу' : needsClosing ? 'закрыть стол' : visitOrders.length ? `${visitOrders.length} заказ${visitOrders.length > 1 ? 'а' : ''}` : 'свободен'}</small>{count > 0 && <b className="table-count-badge">{count}</b>}</button>; })}</div>
        ...
        {selectedClosedHistory && <div className="clear-history-strip"><div><strong>Стол {selectedTable} свободен</strong><small>Архив прошлых заказов остаётся на устройстве</small></div><button onClick={() => { setClearTableId(selectedTable); setClearTablePassword(''); }}>Очистить историю</button></div>}
  ...
  return <main className="app-shell" data-train={state.theme}><div className="brand-ornament" /><header className="app-header"><div className="brand"><div className="brand-mark"><i /><i /></div><div><strong>{themeName.toLocaleUpperCase('ru-RU')}</strong><span>{trainRoute}</span></div></div><nav className="role-nav" aria-label="Роли прототипа">{roles.map((role) => <button key={role.id} className={activeRole === role.id ? 'active' : ''} onClick={() => setActiveRole(role.id)}><span>{role.icon}</span><strong>{role.id}</strong><small>{role.hint}</small>{role.id === 'Официант' && pendingCount + readyCount + openCalls.length > 0 && <b>{pendingCount + readyCount + openCalls.length}</b>}</button>)}</nav><div className="offline-badge"><i />На борту · офлайн</div></header><div className="prototype-note"><span>Интерактивный прототип</span><p>Все роли работают в одной вкладке. Данные сохраняются только в этом браузере.</p></div>
    ...
    {comboModal && comboItem && <div className="modal-layer combo-modal-layer" onClick={() => setComboModal(null)}><section className="combo-modal" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="eyebrow">Настройка комбо</span><h2>{comboItem.name}</h2></div><button className="icon-button" onClick={() => setComboModal(null)}>×</button></div><div className="combo-progress"><span>Выбрано категорий</span><strong>{comboSections.filter((section) => comboDraft[section.id]).length} из {comboSections.length}</strong></div><div className="combo-sections">{comboSections.map((section, index) => <section key={section.id} className="combo-section"><div><span>{index + 1}</span><div><h3>{section.title}</h3><p>{section.hint}</p></div>{comboDraft[section.id] && <b>Выбрано</b>}</div><div className="combo-options-track">{section.options.map((id) => { const item = itemFor(id)!; const selected = comboDraft[section.id] === id; const stopped = state.stopList.includes(id); return <article key={id} className={`${selected ? 'selected' : ''} ${stopped ? 'stopped' : ''}`}><button className="combo-choice" disabled={stopped} onClick={() => setComboDraft((current) => ({ ...current, [section.id]: id }))}><img src={`${basePath}/${item.image}`} alt="" /><span><strong>{item.name}</strong><small>{stopped ? 'Скоро появится' : `≈ ${effectiveMinutes(id)} мин`}</small></span>{selected && <b>✓</b>}</button><button className="combo-choice-info" onClick={() => setInfoItemId(id)} aria-label={`Информация о ${item.name}`}>i</button></article>; })}</div></section>)}</div><div className="combo-modal-footer"><div><span>Стоимость комбо</span><strong>{money(comboItem.price)}</strong></div><button className="primary-button" disabled={!comboSections.every((section) => comboDraft[section.id]) || Object.values(comboDraft).some((id) => state.stopList.includes(id))} onClick={saveCombo}>＋ Добавить комбо в заказ</button></div></section></div>}
    {waiterCheckoutOpen && <div className="modal-layer" onClick={() => setWaiterCheckoutOpen(false)}><section className="waiter-checkout-modal" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="eyebrow">Стол {selectedTable} · место {selectedSeat}</span><h2>Новый заказ</h2></div><button className="icon-button" onClick={() => setWaiterCheckoutOpen(false)}>×</button></div><p className="waiter-checkout-lead">Проверьте состав перед отправкой на кухню и в бар.</p><div className="waiter-checkout-lines">{waiterCheckoutLines.map((line) => { const item = itemFor(line.id)!; const details = Object.values(line.comboSelections ?? {}).map((id) => itemFor(id)?.name).filter(Boolean); return <article key={line.id}><button onClick={() => setInfoItemId(item.id)}><img src={`${basePath}/${item.image}`} alt="" /></button><div><strong>{line.qty}× {item.name}</strong><small>{money(item.price)} · до {effectiveMinutes(item.id)} мин</small>{details.length > 0 && <p>{details.join(' · ')}</p>}</div></article>; })}</div>{waiterComment && <div className="order-comment">Комментарий: “{waiterComment}”</div>}<div className="review-total"><span>{waiterCheckoutLines.reduce((sum, line) => sum + line.qty, 0)} позиций · до {etaForLines(waiterCheckoutLines)} мин</span><strong>{money(totalFor(waiterCheckoutLines))}</strong></div><div className="modal-actions"><button className="secondary-button" onClick={() => setWaiterCheckoutOpen(false)}>Вернуться к заказу</button><button className="primary-button" disabled={!waiterCheckoutLines.length || waiterCheckoutLines.some(lineUnavailable)} onClick={() => createOrder('waiter')}>Отправить в работу</button></div></section></div>}
    {checkoutOpen && <div className="modal-layer" onClick={() => setCheckoutOpen(false)}><section className="checkout-modal" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="eyebrow">Стол 4</span><h2>{passengerHasOpenOrders ? 'Оформление дозаказа' : 'Оформление заказа'}</h2></div><button className="icon-button" onClick={() => setCheckoutOpen(false)}>×</button></div><div className="checkout-summary"><span>{cartCount} позиций · до {etaForLines(passengerCartLines)} мин</span><strong>{money(totalFor(passengerCartLines))}</strong></div><div className="field-group"><label>Ваше место за столом</label><div className="choice-row seats">{[1,2,3,4].map((seat) => <button key={seat} className={passengerSeat === seat ? 'active' : ''} onClick={() => setPassengerSeat(seat)}>{seat}</button>)}</div></div>{passengerHasOpenOrders && passengerVisit ? <div className="inherited-payment"><span>Способ оплаты сохранён для стола</span><strong>{passengerVisit.payment === 'card' ? '▣ Картой' : '▤ Наличными'}</strong></div> : <div className="field-group"><label>Способ оплаты</label><div className="choice-row"><button className={payment === 'card' ? 'active' : ''} onClick={() => setPayment('card')}>▣ Картой</button><button className={payment === 'cash' ? 'active' : ''} onClick={() => setPayment('cash')}>▤ Наличными</button></div></div>}<label className="field-group"><span>Комментарий к заказу</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Например: без лука, приборы на двоих" maxLength={160} /></label><div className="confirmation-note"><span>◇</span><p>{passengerHasOpenOrders ? 'Новые позиции уйдут отдельным дозаказом и не изменят уже готовящиеся блюда.' : state.confirmationRequired ? 'После отправки официант проверит заказ и подтвердит его.' : 'Заказ сразу поступит на кухню и в бар.'}</p></div><button className="primary-button full large" disabled={passengerCartLines.some(lineUnavailable)} onClick={() => createOrder('passenger')}>Отправить {passengerHasOpenOrders ? 'дозаказ' : 'заказ'} · {money(totalFor(passengerCartLines))}</button></section></div>}
    {reviewOrder && <div className="modal-layer" onClick={() => { setReviewId(null); setRejectReason(''); }}><section className="review-modal" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="eyebrow">{reviewOrder.parentId ? `Дозаказ к №${reviewOrder.parentId}` : 'Заказ пассажира'}</span><h2>№{reviewOrder.id} · стол {reviewOrder.table}</h2></div><button className="icon-button" onClick={() => { setReviewId(null); setRejectReason(''); }}>×</button></div><div className="review-meta"><span>Место {reviewOrder.seat}</span><span>{reviewOrder.payment === 'card' ? 'Оплата картой' : 'Оплата наличными'}</span><span>{stamp(new Date(reviewOrder.createdAt))}</span></div>{reviewOrder.comment && <div className="order-comment">Комментарий: “{reviewOrder.comment}”</div>}<div className="review-lines">{reviewLines.map((line) => { const item = itemFor(line.id)!; const stopped = lineUnavailable(line); const details = Object.values(line.comboSelections ?? {}).map((id) => itemFor(id)?.name).filter(Boolean); return <div key={line.id} className={stopped ? 'review-stopped' : ''}><img src={`${basePath}/${item.image}`} alt="" /><div><strong>{item.name}</strong><small>{money(item.price)} · {hasStation([line], 'kitchen') ? 'Кухня' : ''}{hasStation([line], 'kitchen') && hasStation([line], 'bar') ? ' + ' : ''}{hasStation([line], 'bar') ? 'Бар' : ''}{stopped ? ' · В СТОП-ЛИСТЕ' : ''}</small>{details.length > 0 && <p>{details.join(' · ')}</p>}</div><Stepper value={reviewDraft[line.id] ?? 0} onChange={(value) => setReviewDraft((current) => ({ ...current, [line.id]: value }))} /></div>; })}</div><div className="review-total"><span>После подтверждения: до {etaForLines(reviewLines)} мин</span><strong>{money(totalFor(reviewLines))}</strong></div>{reviewLines.some(lineUnavailable) && <div className="warning-note">Одна из позиций или частей комбо находится в стоп-листе.</div>}<label className="reject-reason"><span>Причина отклонения</span><textarea value={rejectReason} onChange={(event) => setRejectReason(event.target.value)} placeholder="Например: позиция закончилась, кухня временно не принимает заказы" maxLength={160} /></label><div className="modal-actions"><button className="danger-button" disabled={!rejectReason.trim()} onClick={rejectReview}>Отклонить</button><button className="primary-button" disabled={!reviewLines.length || reviewLines.some(lineUnavailable)} onClick={approveReview}>Подтвердить и отправить</button></div></section></div>}
    {infoItem && <div className="modal-layer" onClick={() => setInfoItemId(null)}><section className="info-modal" onClick={(event) => event.stopPropagation()}><div className="info-modal-visual"><img src={`${basePath}/${infoItem.image}`} alt={infoItem.name} /><span>{infoItem.category} · {infoItem.weight}</span></div><div className="info-modal-copy"><div className="panel-heading"><div><span className="eyebrow">Состав и пищевая ценность</span><h2>{infoItem.name}</h2></div><button className="icon-button" onClick={() => setInfoItemId(null)}>×</button></div><p>{infoItem.ingredients}</p><div className="nutrition-grid"><div><strong>{infoItem.calories}</strong><span>ккал</span></div><div><strong>{infoItem.protein}</strong><span>белки, г</span></div><div><strong>{infoItem.fat}</strong><span>жиры, г</span></div><div><strong>{infoItem.carbs}</strong><span>углеводы, г</span></div></div><small>Пищевая ценность указана на порцию и носит справочный характер.</small>{infoItem.modifiers.length > 0 && <div className="info-modifiers"><span>Можно выбрать</span>{infoItem.modifiers.map((modifier) => <b key={modifier}>{modifier}</b>)}</div>}<button className="primary-button full" onClick={() => setInfoItemId(null)}>Понятно</button></div></section></div>}
    {clearTableId && <div className="modal-layer" onClick={() => setClearTableId(null)}><section className="password-modal" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="eyebrow">Доступ официанта</span><h2>Очистить историю стола {clearTableId}</h2></div><button className="icon-button" onClick={() => setClearTableId(null)}>×</button></div><p>Будут удалены завершённые заказы и принятые вызовы этого стола. Активные заказы не затрагиваются.</p><label><span>Пароль</span><input type="password" value={clearTablePassword} onChange={(event) => setClearTablePassword(event.target.value)} placeholder="Введите пароль" autoFocus onKeyDown={(event) => { if (event.key === 'Enter') clearTableHistory(); }} /></label><small>Для демонстрации: пароль 7420</small><button className="danger-button full" onClick={clearTableHistory}>Очистить историю</button></section></div>}
